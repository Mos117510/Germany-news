...
async function zdfLinks() {
  ...
  const seen=new Set();
  return out.filter(x=>{if(seen.has(x.link)) return false; seen.add(x.link); return true;}).slice(0,35);
}
...
async function articleExtract(item) {
  try {
    ...
    return {...item,description:clean(desc)||item.description,content:paras.slice(0,18).join(' ')};
  } catch { return item; }
}
...
async function buildUpdate(env, day, previous){
  ...
  try{ts=await rssItems();}catch(e){failures.push('tagesschau.de');}
  try{zdf=await zdfLinks();}catch(e){failures.push('zdfheute.de');}
  ...
  const raw=unique([...ts.slice(0,18),...zdf.slice(0,18)]);
  if (!raw.length) return {noNews:true, message: previous ? 'No New News yet' : 'No News', failures};
  const enriched=[]; for(let i=0;i<Math.min(raw.length,24);i++) enriched.push(await articleExtract(raw[i]));
  const oldLinks=new Set((previous?.articles||[]).map(x=>x.link));
  const ai=await env.AI.run('@cf/google/gemma-4-26b-a4b-it',{messages:[{role:'user',content:promptFor(enriched,day)}]});
  ...
  const allArticles=enriched.map(a=>({source:a.source,title:a.title,link:a.link}));
